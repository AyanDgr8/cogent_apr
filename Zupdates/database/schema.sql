...
CREATE TABLE IF NOT EXISTS agent_activity (
    ...
    -- Multiple state transitions can legitimately share a timestamp (for example
    -- agent_on_call=true and agent_idle=false). Keep those distinct while still
    -- making repeated API ingestion idempotent.
    UNIQUE KEY unique_agent_activity (
        agent_name,
        event_timestamp,
        event_type,
        event_state
...
CREATE TABLE IF NOT EXISTS agent_complete_hourly (
    ...
    available_time VARCHAR(20) DEFAULT '00:00:00' COMMENT 'Registered time minus not available time',
