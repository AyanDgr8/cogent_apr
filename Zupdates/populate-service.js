// populate-service.js
// A robust service script to automatically populate the SPC agent database at regular intervals
// Usage: node spc-populate-service.js [intervalMinutes] [lookbackHours]
// Example: node spc-populate-service.js 5 1
...
const LOG_FILE = path.join(process.cwd(), 'spc-populate-service.log');
const STATUS_FILE = path.join(process.cwd(), 'spc-populate-status.json');
...
// Main function to populate SPC agent database
async function populateSPCDatabase() {
  const startTime = Date.now();
  const runId = `spc-run-${Date.now()}`;
  log(`Starting SPC agent database population process (ID: ${runId})...`);
  ...
  try {
    ...
    log(`🎉 SPC agent database population completed successfully in ${duration}s!`, 'success');
    ...
    log(`Error in SPC database population process (ID: ${runId}): ${error.message}`, 'error');
...
function scheduleNextRun() {
  ...
  log(`⏰ Next SPC population scheduled for: ${dubaiTime} Dubai time (in ${config.intervalMinutes} minutes)`, 'success');
  ...
  setTimeout(() => {
    log('🚀 Starting scheduled SPC population run...', 'info');
    runService();
  }, intervalMs);
}
...
function initializeLogFile() {
  const header = `
=========================================
SPC AGENT DATABASE POPULATE SERVICE STARTED
=========================================
...
(async () => {
  ...
  // Start the service
  log('Starting initial SPC agent database population run...');
  await runService();
})();

// Handle graceful shutdown
process.on('SIGINT', () => {
  log('Received SIGINT. Shutting down SPC populate service gracefully...', 'warning');
  updateStatusFile({
    status: 'stopped',
...
process.on('SIGTERM', () => {
  log('Received SIGTERM. Shutting down SPC populate service gracefully...', 'warning');
  updateStatusFile({
    status: 'stopped',
