...
import { TENANT_CONFIG, getTenantConfig } from './tenantConfig.js';
...
/**
 * Build authentication headers for report APIs that support tenant API keys.
 * This bypasses portal login and does not use username/password credentials.
 */
export function getApiKeyHeaders(tenant) {
  const { api_key: apiKey, account_id: accountId } = getTenantConfig(tenant);

  if (!apiKey) {
    throw new Error(`API key is not configured for tenant: ${tenant}`);
  }
  ...
  for (let i = 0, delay = 1000; i < MAX_RETRIES; i++, delay *= 2) {
    try {
      const {data} = await axios.post(
        ...
        { username: process.env.API_USERNAME, password: process.env.API_PASSWORD, domain },
...
export async function getPortalToken(tenant) {
  ...
  const candidates = [
    ...
    { url: `${base_url}/api/v2/config/login/oauth`, body: { domain, username: process.env.API_USERNAME, password: process.env.API_PASSWORD } },
    // v2 login using domain (fallback for older back-ends)
    { url: `${base_url}/api/v2/login`, body: { domain, username: process.env.API_USERNAME, password: process.env.API_PASSWORD } },
    // very old legacy login path
    { url: `${base_url}/api/login`, body: { domain, username: process.env.API_USERNAME, password: process.env.API_PASSWORD } },
  ];
